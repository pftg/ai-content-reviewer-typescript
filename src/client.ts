...
export interface ClientOptions {
  /**
   * Enter your API key in the format: Bearer sk_... or just sk_...
   */
   ...
   *
   * Defaults to process.env['AI_CONTENT_REVIEWER_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['AI_CONTENT_REVIEWER_LOG'] or 'warn' if it isn't set.
   */
...
export class AIContentReviewer {
   ...
   */
  constructor({
    baseURL = readEnv('AI_CONTENT_REVIEWER_BASE_URL'),
    apiKey = readEnv('AI_CONTENT_REVIEWER_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.AIContentReviewerError(
        "The AI_CONTENT_REVIEWER_API_KEY environment variable is missing or empty; either provide it, or instantiate the AIContentReviewer client with an apiKey option, like new AIContentReviewer({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? AIContentReviewer.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('AI_CONTENT_REVIEWER_LOG'), "process.env['AI_CONTENT_REVIEWER_LOG']", this) ??
      defaultLogLevel;
  ...
  static AIContentReviewer = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static AIContentReviewerError = Errors.AIContentReviewerError;
