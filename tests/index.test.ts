...
import { APIPromise } from 'ai-content-reviewer/core/api-promise';

import util from 'node:util';
import AIContentReviewer from 'ai-content-reviewer';
import { APIUserAbortError } from 'ai-content-reviewer';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['AI_CONTENT_REVIEWER_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: AIContentReviewer) => {
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['AI_CONTENT_REVIEWER_LOG'] = 'debug';
      const client = new AIContentReviewer({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['AI_CONTENT_REVIEWER_LOG'] = 'not a log level';
      const client = new AIContentReviewer({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'AI_CONTENT_REVIEWER_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['AI_CONTENT_REVIEWER_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new AIContentReviewer({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['AI_CONTENT_REVIEWER_BASE_URL'] = 'https://example.com/from_env';
      const client = new AIContentReviewer({ apiKey: 'My API Key' });
    ...
    test('in request options overridden by client options', () => {
      const client = new AIContentReviewer({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['AI_CONTENT_REVIEWER_BASE_URL'] = 'http://localhost:5000/env';
      const client = new AIContentReviewer({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new AIContentReviewer({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new AIContentReviewer({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['AI_CONTENT_REVIEWER_API_KEY'] = 'My API Key';
    const client = new AIContentReviewer();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['AI_CONTENT_REVIEWER_API_KEY'] = 'another My API Key';
    const client = new AIContentReviewer({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new AIContentReviewer({ apiKey: 'My API Key', fetch: testFetch });
